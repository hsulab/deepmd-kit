...
REGISTER_OP("ProdForceSeA")
    .Attr("T: {float, double}")
    .Input("net_deriv: T")  // jx: dEi/dRi=dEi/dDi*dDi/dRi
    .Input("in_deriv: T")   // jx: dRi/dr_nei
    .Input("nlist: int32")
    .Input("natoms: int32")
    .Attr("n_a_sel: int")
    .Attr("n_r_sel: int")
...
class ProdForceSeAOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(context, (0 == n_r_sel),
                errors::InvalidArgument(
                    "Rotational free only support all-angular information"));
...
#pragma omp parallel for
    for (int kk = 0; kk < nframes; ++kk) {
      ...
      } // jx: init force tensor with zeros 
      ...
      for (int ii = 0; ii < nloc; ++ii) {
        ...
        for (int jj = 0; jj < nnei; ++jj) {
          ...
          if (j_idx < 0) continue;
          int aa_start, aa_end; // jx: positive due to x_j-x_i
 ...
 private:
  ...
  inline void make_descript_range(int& idx_start, int& idx_end,
                                  const int& nei_idx) {
    if (nei_idx < n_a_sel) {
      idx_start = nei_idx * 4;
      idx_end = nei_idx * 4 + 4;
