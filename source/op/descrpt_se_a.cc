...
REGISTER_OP("DescrptSeA")
    ...
    .Input("natoms: int32")  // local atomic number; each type atomic number;
                             // daizheyingxiangqude atomic numbers
    .Input("box : T")
    .Input("mesh : int32")
...
class DescrptSeAOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    for (int kk = 0; kk < nsamples;
         ++kk) {  // jx: main loop over all configurations
      // set region
...
#pragma omp parallel for
      for (int ii = 0; ii < nloc; ++ii) {
        ...
        int ret = -1;
        if (fill_nei_a) {
          if ((ret = format_nlist_fill_a(fmt_nlist_a, fmt_nlist_r, d_coord3,
                                         ntypes, d_type, region, b_pbc, ii,
                                         d_nlist_a[ii], d_nlist_r[ii], rcut_r,
                                         sec_a, sec_r)) != -1) {
            if (count_nei_idx_overflow == 0) {
              cout << "WARNING: Radial neighbor list length of type " << ret
                   << " is not enough" << endl;
              flush(cout);
        ...
        compute_descriptor_se_a(d_descrpt_a, d_descrpt_a_deriv, d_rij_a,
                                d_coord3, ntypes, d_type, region, b_pbc, ii,
                                fmt_nlist_a, sec_a, rcut_r_smth, rcut_r);
 ...
 private:
  ...
  void cum_sum(vector<int>& sec, const vector<int32>& n_sel) const {
