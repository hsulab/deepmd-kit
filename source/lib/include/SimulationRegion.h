...
class SimulationRegion {
 ...
 public:
  ...
  void shiftCoord(const int *idx, VALUETYPE &x, VALUETYPE &y,
                  VALUETYPE &z) const;
  static int getNumbShiftVec() { return shift_info_size; }
  static int getShiftVecTotalSize() { return shift_vec_size; }

 public:
  void diffNearestNeighbor(const VALUETYPE *r0, const VALUETYPE *r1,
                           VALUETYPE *phys) const;
  virtual void diffNearestNeighbor(const VALUETYPE x0, const VALUETYPE y0,
                                   const VALUETYPE z0, const VALUETYPE x1,
                                   const VALUETYPE y1, const VALUETYPE z1,
                                   VALUETYPE &dx, VALUETYPE &dy,
                                   VALUETYPE &dz) const;
  virtual void diffNearestNeighbor(const VALUETYPE x0, const VALUETYPE y0,
                                   const VALUETYPE z0, const VALUETYPE x1,
                                   const VALUETYPE y1, const VALUETYPE z1,
                                   VALUETYPE &dx, VALUETYPE &dy, VALUETYPE &dz,
 ...
 private:
  ...
  void naiveTensorDotVector(double *out, const double *i_t,
                            const double *i_v) const;
  void naiveTensorTransDotVector(double *out, const double *i_t,
                                 const double *i_v) const;
  void tensorDotVector(double *out, const double *i_t, const double *i_v) const;
  void tensorTransDotVector(double *out, const double *i_t,
                            const double *i_v) const;
