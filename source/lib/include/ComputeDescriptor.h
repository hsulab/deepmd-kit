...
inline int format_nlist_fill_a(
    vector<int> &fmt_nei_idx_a, vector<int> &fmt_nei_idx_r,
    const vector<double> &posi, const int &ntypes, const vector<int> &type,
    const SimulationRegion<double> &region, const bool &b_pbc, const int &i_idx,
    const vector<int> &nei_idx_a, const vector<int> &nei_idx_r,
    const double &rcut, const vector<int> &sec_a, const vector<int> &sec_r);

inline void compute_descriptor(
    vector<double> &descrpt_a, vector<double> &descrpt_r,
    vector<double> &rot_mat, const vector<double> &posi, const int &ntypes,
    const vector<int> &type, const SimulationRegion<double> &region,
    const bool &b_pbc, const int &i_idx, const vector<int> &fmt_nlist_a,
    const vector<int> &fmt_nlist_r, const vector<int> &sec_a,
    const vector<int> &sec_r, const int axis0_type, const int axis0_idx,
    const int axis1_type, const int axis1_idx);

inline void compute_descriptor(
    vector<double> &descrpt_a, vector<double> &descrpt_a_deriv,
    vector<double> &descrpt_r, vector<double> &descrpt_r_deriv,
    vector<double> &rij_a, vector<double> &rij_r, vector<double> &rot_mat,
    const vector<double> &posi, const int &ntypes, const vector<int> &type,
    const SimulationRegion<double> &region, const bool &b_pbc, const int &i_idx,
    const vector<int> &fmt_nlist_a, const vector<int> &fmt_nlist_r,
    const vector<int> &sec_a, const vector<int> &sec_r, const int axis0_type,
    const int axis0_idx, const int axis1_type, const int axis1_idx);

inline void compute_descriptor_se_a(
    vector<double> &descrpt_a, vector<double> &descrpt_a_deriv,
    vector<double> &rij_a, const vector<double> &posi, const int &ntypes,
    const vector<int> &type, const SimulationRegion<double> &region,
    const bool &b_pbc, const int &i_idx, const vector<int> &fmt_nlist_a,
    const vector<int> &sec_a, const double &rmin, const double &rmax);

inline void compute_descriptor_se_r(
    vector<double> &descrpt_r, vector<double> &descrpt_r_deriv,
    vector<double> &rij_r, const vector<double> &posi, const int &ntypes,
    const vector<int> &type, const SimulationRegion<double> &region,
    const bool &b_pbc, const int &i_idx, const vector<int> &fmt_nlist_r,
    const vector<int> &sec_r, const double &rmin, const double &rmax);
...
static void compute_dRdT(double (*dRdT)[9], const double *r1, const double *r2,
                         const double *rot) {
  double *dRdT0 = dRdT[0];
  double *dRdT1 = dRdT[1];
  double *dRdT2 = dRdT[2];
...
static void compute_dRdT_1(double (*dRdT)[9], const double *r1,
                           const double *r2, const double *rot) {
  double *dRdT0 = dRdT[0];
  double *dRdT1 = dRdT[1];
  double *dRdT2 = dRdT[2];
  const double *xx = rot;
...
static void compute_dRdT_2(double (*dRdT)[9], const double *r1,
                           const double *r2, const double *rot) {
  double *dRdT0 = dRdT[0];
  double *dRdT1 = dRdT[1];
  double *dRdT2 = dRdT[2];
...
int format_nlist_fill_a(vector<int> &fmt_nei_idx_a, vector<int> &fmt_nei_idx_r,
                        const vector<double> &posi, const int &ntypes,
                        const vector<int> &type,
                        const SimulationRegion<double> &region,
                        const bool &b_pbc, const int &i_idx,
                        const vector<int> &nei_idx_a,
                        const vector<int> &nei_idx_r, const double &rcut,
                        const vector<int> &sec_a, const vector<int> &sec_r) {
#ifdef DEBUG
...
void compute_descriptor(
  ...
  // 	  cout << "\t " << sqrt(MathUtilities::dot (&sel_a_diff[jj][0],
  // &sel_a_diff[jj][0]));
  // 	}
  // 	cout << endl;
  //     }
  ...
  // 	if (j_idx >= 0){
  // 	  cout << "\t type " << type[j_idx];
  // 	  cout << "\t " << sqrt(MathUtilities::dot (&sel_r_diff[jj][0],
  // &sel_r_diff[jj][0]));
  // 	}
  // 	cout << endl;
  //     }
...
void compute_descriptor(vector<double> &descrpt_a, vector<double> &descrpt_r,
                        vector<double> &rot_mat, const vector<double> &posi,
                        const int &ntypes, const vector<int> &type,
                        const SimulationRegion<double> &region,
                        const bool &b_pbc, const int &i_idx,
                        const vector<int> &fmt_nlist_a,
                        const vector<int> &fmt_nlist_r,
                        const vector<int> &sec_a, const vector<int> &sec_r,
                        const int axis0_type, const int axis0_idx,
                        const int axis1_type, const int axis1_idx) {
  // compute the diff of the neighbors
  vector<vector<double> > sel_a_diff(sec_a.back());
  for (int ii = 0; ii < int(sec_a.size()) - 1; ++ii) {
...
inline double cos_switch(const double &xx, const double &rmin,
                         const double &rmax) {
  if (xx < rmin) {
    return 1.;
  } else if (xx < rmax) {
...
inline void cos_switch(double &vv, double &dd, const double &xx,
...
inline void spline3_switch(double &vv, double &dd, const double &xx,
                           const double &rmin, const double &rmax) {
  if (xx < rmin) {
    dd = 0;
    vv = 1;
...
inline void spline5_switch(TYPE &vv, TYPE &dd, const TYPE &xx, const TYPE &rmin,
                           const TYPE &rmax) {
  if (xx < rmin) {
    dd = 0;
    vv = 1;
...
void compute_descriptor_se_r(
    vector<double> &descrpt, vector<double> &descrpt_deriv, vector<double> &rij,
    const vector<double> &posi, const int &ntypes, const vector<int> &type,
    const SimulationRegion<double> &region, const bool &b_pbc, const int &i_idx,
    const vector<int> &fmt_nlist, const vector<int> &sec, const double &rmin,
    const double &rmax) {
  // compute the diff of the neighbors
