...
template <typename VALUETYPE>
inline void SimulationRegion<VALUETYPE>::apply_periodic(int dim, double *dd,
                                                        int &shift) const {
  shift = 0;
  if (!is_periodic[dim]) return;
  if (dd[dim] >= static_cast<double>(0.5)) {
...
template <typename VALUETYPE>
inline void SimulationRegion<VALUETYPE>::diffNearestNeighbor(
    const VALUETYPE x0, const VALUETYPE y0, const VALUETYPE z0,
    const VALUETYPE x1, const VALUETYPE y1, const VALUETYPE z1, VALUETYPE &dx,
    VALUETYPE &dy, VALUETYPE &dz) const {
...
inline void SimulationRegion<VALUETYPE>::diffNearestNeighbor(
    const VALUETYPE x0, const VALUETYPE y0, const VALUETYPE z0,
    const VALUETYPE x1, const VALUETYPE y1, const VALUETYPE z1, VALUETYPE &dx,
    VALUETYPE &dy, VALUETYPE &dz, int &shift_x, int &shift_y,
    int &shift_z) const {
...
inline void SimulationRegion<VALUETYPE>::diffNearestNeighbor(
    const VALUETYPE x0, const VALUETYPE y0, const VALUETYPE z0,
    const VALUETYPE x1, const VALUETYPE y1, const VALUETYPE z1, VALUETYPE &dx,
    VALUETYPE &dy, VALUETYPE &dz, VALUETYPE &shift_x, VALUETYPE &shift_y,
...
inline void SimulationRegion<VALUETYPE>::toFaceDistance(double *dd) const {
  ...
  dd[0] = volume * MathUtilities::invsqrt(MathUtilities::dot(
                       tmp, tmp));  // jx: vertial distance to y-z plane
...
inline void SimulationRegion<VALUETYPE>::computeVolume() {
  ...
  if (volume < 0) {
    throw std::runtime_error(
        "Negative volume detected. Please make sure the simulation cell obeys "
        "the right-hand rule.");
  }
}
