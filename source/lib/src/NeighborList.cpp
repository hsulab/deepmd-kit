...
bool is_loc(const vector<int>& idx, const vector<int>& nat_stt,
...
int collapse_index(const vector<int>& idx, const vector<int>& size) {
  return (idx[0] * size[1] + idx[1]) * size[2] + idx[2];
}

void expand_index(vector<int>& o_idx, const int& i_idx,
...
void build_clist(vector<vector<int> >& clist, const vector<double>& coord,
                 const int& nloc, const vector<int>& nat_stt,
                 const vector<int>& nat_end, const vector<int>& ext_stt,
    ...
    for (int dd = 0; dd < 3; ++dd) {
      ...
      if (idx[dd] < nat_stt[dd]) {
        if (count_warning_loc_idx_lower < MAX_WARN_IDX_OUT_OF_BOUND) {
          cerr << "# warning: loc idx out of lower bound (ignored if warned "
                  "for more than "
               << MAX_WARN_IDX_OUT_OF_BOUND << " times) " << endl;
          count_warning_loc_idx_lower++;
        }
        idx[dd] = nat_stt[dd];
      } else if (idx[dd] >= nat_end[dd]) {
        if (count_warning_loc_idx_upper < MAX_WARN_IDX_OUT_OF_BOUND) {
          cerr << "# warning: loc idx out of upper bound (ignored if warned "
                  "for more than "
               << MAX_WARN_IDX_OUT_OF_BOUND << " times) " << endl;
    ...
    for (int dd = 0; dd < 3; ++dd) {
      ...
      if (idx[dd] < ext_stt[dd]) {
        if (count_warning_ghost_idx_lower < MAX_WARN_IDX_OUT_OF_BOUND &&
            fabs((inter[dd] - nat_orig[dd]) - (ext_stt[dd] * cell_size[dd])) >
                fabs(ext_stt[dd] * cell_size[dd]) *
                    numeric_limits<double>::epsilon() * 5.) {
          cerr << "# warning: ghost idx out of lower bound (ignored if warned "
                  "for more than "
               << MAX_WARN_IDX_OUT_OF_BOUND << " times) " << endl;
        ...
        if (count_warning_ghost_idx_upper < MAX_WARN_IDX_OUT_OF_BOUND) {
          cerr << "# warning: ghost idx out of upper bound (ignored if warned "
                  "for more than "
               << MAX_WARN_IDX_OUT_OF_BOUND << " times) " << endl;
...
void build_clist(vector<vector<int> >& clist, const vector<double>& coord,
                 const vector<int>& sel, const vector<int>& nat_stt,
                 const vector<int>& nat_end,
                 const SimulationRegion<double>& region) {
  static int count_warning_loc_idx_lower = 0;
  static int count_warning_loc_idx_upper = 0;
...
void build_nlist_cell(vector<vector<int> >& nlist0,
                      vector<vector<int> >& nlist1, const int& cidx,
                      const int& tidx, const vector<vector<int> >& clist,
                      const vector<double>& coord, const double& rc02,
                      const double& rc12, const vector<int>& shift = {0, 0, 0},
                      const vector<double>& boxt = {0., 0., 0., 0., 0., 0., 0.,
                                                    0., 0.}) {
...
void build_nlist_cell(vector<vector<int> >& nlist0,
                      vector<vector<int> >& nlist1, const int& cidx,
                      const int& tidx, const vector<vector<int> >& clist0,
...
void build_nlist(vector<vector<int> >& nlist0, vector<vector<int> >& nlist1,
                 const vector<double>& coord, const int& nloc,
                 const double& rc0, const double& rc1,
                 const vector<int>& nat_stt_, const vector<int>& nat_end_,
                 const vector<int>& ext_stt_, const vector<int>& ext_end_,
                 const SimulationRegion<double>& region,
                 const vector<int>& global_grid) {
  // normalize the index
...
void build_nlist(vector<vector<int> >& nlist0, vector<vector<int> >& nlist1,
                 const vector<double>& coord, const double& rc0,
                 const double& rc1, const vector<int>& grid,
                 const SimulationRegion<double>& region) {
  // assuming nloc == nall
...
void build_nlist(vector<vector<int> >& nlist0, vector<vector<int> >& nlist1,
                 const vector<double>& coord, const vector<int>& sel0,
                 const vector<int>& sel1, const double& rc0, const double& rc1,
...
void build_nlist(vector<vector<int> >& nlist0, vector<vector<int> >& nlist1,
                 const vector<double>& posi3, const double& rc0_,
                 const double& rc1_, const SimulationRegion<double>* region) {
  double rc0(rc0_);
  double rc1(rc1_);
  assert(rc0 <= rc1);
  double rc02 = rc0 * rc0;
...
void copy_coord(vector<double>& out_c, vector<int>& out_t, vector<int>& mapping,
                vector<int>& ncell, vector<int>& ngcell,
                const vector<double>& in_c, const vector<int>& in_t,
                const double& rc, const SimulationRegion<double>& region) {
  int nloc = in_c.size() / 3;   // jx: number of atoms
  assert(nloc == in_t.size());  // jx: equals number of types
  ...
  for (int dd = 0; dd < 3; ++dd) {
    ncell[dd] =
        to_face[dd] / rc;  // jx: check the times of vertial plane distance
  ...
  int esti_ntotal =
      total_ncell / loc_ncell * nloc + 10;  // jx: number of extended atoms
